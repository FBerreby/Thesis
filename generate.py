...
# i(benCosts(s1,S,i1,I,t1,T,n1,N))
				if 'i(benCosts' in line: 
					...
					output.write ("is impermissible acording to the principle of Benefits Vs. Costs because its compared weight is negative"+cw.group(1)) 
# p(benCosts(s1,S,i1,I,t1,T,n1,N))
					...
					output.write ("\n which occurs in simulation"+sim.group(1))
					output.write ("at time"+time.group(1))
					output.write ("is permissible acording to the principle of Benefits Vs. Costs because its compared weight is positive:"+cw.group(1)) 
...
# i(actUti(s1,S1,i1,I1,t1,T,n1,N1,i2,I2,n2,N2))
					...
					output.write ("\n which occurs in simulation"+sim.group(1))
					output.write ("at time"+time.group(1))
					output.write ("is impermissible acording to Act Utilitarianism because it does not lead to the best possible outcome. Its best possible outcome has an added weight of"+aw.group(1))
...
# p(actUti(s1,S,i1,I,t1,T,n1,N))
					...
					output.write ("\n which occurs in simulation"+sim.group(1))
					output.write ("at time"+time.group(1))
					output.write ("is permissible acording to Act Utilitarianism because it can lead to the best possible outcome, whose added weight is:"+aw.group(1)) 
...
# i(leastBad(s1,S,i1,I,t1,T,n1,N))
					...
					output.write ("\n which occurs in simulation"+sim.group(1))
					output.write ("at time"+time.group(1))
					output.write ("is impermissible acording to the Principle of Least Bad consequence because its worst possible consequence, whose added weight is"+aw.group(1)+"is worse that the worst possible consequence of at least one of its viable alternatives.")
# p(leastBad(s1,S,i1,I,t1,T,n1,N))
					...
					output.write ("\n which occurs in simulation"+sim.group(1))
					output.write ("at time"+time.group(1))
					output.write ("is permissible acording to the Principle of Least Bad consequence because its worst possible consequence, whose added weight is"+aw.group(1)+"is the best among the worst possible consequences of its viable alternatives.")
...
# i(pureBad(s1,S,i1,I,t1,T))					
					...
					output.write ("\n which occurs in simulation"+sim.group(1))
					output.write ("at time"+time.group(1))
					output.write ("is impermissible acording to the rule of Prohibiting Purely Bad volitions because it leads only to negative outcomes.") 
# p(pureBad(s1,S,i1,I,t1,T))										
					...
					output.write ("\n which occurs in simulation"+sim.group(1))
					output.write ("at time"+time.group(1))
					output.write ("is permissible acording to the rule of Prohibiting Purely Bad volitions because it does not lead only to negative outcomes.") 
...
# i(ruleUti(s1,S1,i1,I1,t1,T,n1,N1,r1,R1,i2,I2,n2,N2,r2,R2))
					...
					output.write ("\n which occurs in simulation"+sim.group(1))
					output.write ("at time"+time.group(1))
					output.write ("is impermissible acording to Rule Utilitarianism because it is an instance of the rule"+rule1.group(1))
...
# p(ruleUti(s1,S,i1,I,t1,T,n1,N,r1,R)):-per(ruleUti,S,T,I),instance(I,R),ruleWeight(R,N).
					...
					output.write ("\n which occurs in simulation"+sim.group(1))
					output.write ("at time"+time.group(1))
					output.write ("is permissible acording to Rule Utilitarianism because it is an instance of the rule"+rule1.group(1))
					output.write ("whose rule weight is"+rw1.group(1))
...
# i(conduct(s1,S,i1,I,t1,T))
					...
					output.write ("\n which occurs in simulation"+sim.group(1))
					output.write ("at time"+time.group(1))
					output.write ("is impermissible acording to the implemented Code of Conduct, because it infringes it.") 										
# p(conduct(s1,S,i1,I,t1,T)):-per(conduct,S,T,I).
					...
					output.write ("\n which occurs in simulation"+sim.group(1))
					output.write ("at time"+time.group(1))
					output.write ("is permissible acording to the implemented Code of Conduct, because it does not infringe it.")
...
# i(kant(s1,S,i1,I,t1,T,g1,G,e1,E1))
					...
					output.write ("\n which occurs in simulation"+sim.group(1))
					output.write ("at time"+time.group(1))
					output.write ("is impermissible acording to Kantian ethics because, without aiming for it, it causes event"+event.group(1)) 	
...
# i(kant(s1,S,i1,I,t1,T,g1,G,e1,E,n1,N))
					...
					output.write ("\n which occurs in simulation"+sim.group(1))
					output.write ("at time"+time.group(1))
					output.write ("is impermissible acording to Kantian ethics because it purposefully causes the bad event"+event.group(1)) 	
					output.write ("which impacts on agent(s)"+agent.group(1)) 	
...
# p(kant(s1,S,a1,A,t1,T,g1,G,e1,E,n1,N)):-r(S,causes,A,T,E),impact(E,G,M,N),aim(A,E),weight(E,N),N>0,agent(G),action(A).
					...
					output.write ("\n which occurs in simulation"+sim.group(1))
					output.write ("at time"+time.group(1))
					output.write ("is permissible acording to the Kantian ethics, because ...!!!!") 	
...
# p(dde(s1,S,i1,I,t1,T))
					...
					output.write ("\n which occurs in simulation"+sim.group(1))
					output.write ("at time"+time.group(1))
					output.write ("is permissible acording to the Doctrine of Double Effect because it does not violate any of its conditions.") 
